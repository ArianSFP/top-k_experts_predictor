...
def test_post_moe_probe_geometry_content_gain_and_gradients() -> None:
    ...
    router_inputs = torch.randn_like(states)
    ids = torch.randint(0, 7, (2, 3, 4, 2))
    weights = torch.rand(2, 3, 4, 2)
    combined = torch.cat((states, router_inputs), dim=-1)
    full = probe(combined, ids, weights)
    ablated = probe(combined, ids, weights, use_router_blind_content=False)
