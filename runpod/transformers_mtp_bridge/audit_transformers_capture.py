...
def audit_target(store: Store) -> dict[str, Any]:
    ...
    if not rows:
        raise AuditError("no target layer rows")
    keys = set()
    ...
    for row in rows:
        ...
        check_tensor_roles(store, event_id, TARGET_REQUIRED)
        for role in TARGET_REQUIRED:
            bf16_role_counts[(role, store.tensors[event_id][role]["native_dtype"])] += 1

        x = store.tensor(event_id, "block_input_residual_x")
        ...
        routed_errors.append(rel_rms(complete, routed + shared))
        if np.allclose(a, 0):
            raise AuditError(f"all-zero router input {key}")

        logits = store.tensor(event_id, "raw_target_router_logits").astype(np.float32)
        probs = store.tensor(event_id, "full_target_router_probabilities").astype(np.float32)
        ...
        weights = store.tensor(event_id, "selected_execution_weights").astype(np.float32)
        if len(set(ids.tolist())) != 8 or np.any(ids < 0) or np.any(ids >= 256):
            raise AuditError(f"invalid selected IDs {key}")
        torch_ids = torch.topk(torch.from_numpy(probs), 8).indices.numpy()
        if set(torch_ids.tolist()) != set(ids.tolist()):
            selected_min = float(probs[ids].min())
            unselected = np.delete(probs, ids)
            if selected_min + 1e-7 < float(unselected.max()):
                selected_set_failures += 1
        expected_weights = probs[ids] / probs[ids].sum()
        weight_errors.append(float(np.max(np.abs(expected_weights - weights))))

        unweighted = store.tensor(event_id, "candidate_unweighted_output_vectors")
        weighted = store.tensor(event_id, "candidate_weighted_output_vectors")
        weighted_errors.append(rel_rms(weighted, unweighted * weights[:, None]))
        routed_errors.append(rel_rms(routed, weighted.sum(axis=0)))
        shared_raw = store.tensor(event_id, "shared_expert_output_before_gate")
        gate_logit = float(store.tensor(event_id, "shared_expert_gate_logit")[0])
        gate = 1.0 / (1.0 + math.exp(-gate_logit))
        shared_errors.append(rel_rms(shared, shared_raw * gate))

        if [int(v) for v in ids.tolist()] != [int(v) for v in row["candidate_expert_ids"]]:
            raise AuditError(f"candidate ID alignment mismatch {key}")
        u_rms = float(np.sqrt(np.mean(u.astype(np.float64) ** 2)))
        scalar_errors.append(abs(u_rms - float(row["post_mixer_pre_moe_residual_rms"])))
        wrms = np.sqrt(np.mean(weighted.astype(np.float64) ** 2, axis=-1))
        fractions = wrms / max(float(wrms.sum()), 1e-12)
        scalar_errors.extend(
            np.abs(fractions - np.asarray(row["candidate_weighted_output_fraction"])).tolist()
        )
        for index in range(8):
            ...
            scalar_errors.append(abs(value - float(row["candidate_output_cosine_to_routed_sum"][index])))
    ...
    thresholds = {
        ...
        "complete_moe_relative_rms_max": max(routed_errors),
    ...
    if thresholds["residual_relative_rms_max"] > 0.03:
        raise AuditError(f"target residual identity failed: {thresholds}")
    if thresholds["complete_moe_relative_rms_max"] > 0.03:
        raise AuditError(f"target MoE decomposition failed: {thresholds}")
    if thresholds["candidate_weight_relative_rms_max"] > 0.01:
        raise AuditError(f"candidate weighting failed: {thresholds}")
    if thresholds["shared_relative_rms_max"] > 0.01:
        raise AuditError(f"shared gate identity failed: {thresholds}")
    if thresholds["router_probability_abs_max"] > 2e-6:
        raise AuditError(f"router probability audit failed: {thresholds}")
    if selected_set_failures:
        raise AuditError(f"native selected-set audit failed: {thresholds}")
    if thresholds["selected_weight_abs_max"] > 0.002:
        raise AuditError(f"selected weight audit failed: {thresholds}")
    if thresholds["scalar_abs_max"] > 2e-4:
        raise AuditError(f"target reduction audit failed: {thresholds}")

    return {
        "target_layer_rows": len(rows),
        "authoritative_target_positions": len(layers_by_position),
        "positions_with_all_40_layers": len(layers_by_position),
        "identity_metrics": thresholds,
        "native_dtype_counts": {
            f"{role}:{dtype}": count
            for (role, dtype), count in sorted(bf16_role_counts.items())
        },
    }
...
def audit(root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        if manifest.get("production_capture") is not False:
            raise AuditError("pilot unexpectedly marked production")
        header = store.rows[0]
        if (
        ...
        result = {
            "schema": "gcrp2r_transformers_joint_capture_audit_v1",
            "passed": True,
