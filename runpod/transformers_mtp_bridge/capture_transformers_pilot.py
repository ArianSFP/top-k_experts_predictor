...
class TargetCaptureHooks(AbstractContextManager):
    ...
    def __enter__(self):
        for layer_id, layer in enumerate(self.model.model.layers):
            ...
            def experts_post(module, args, output, row=row):
                hidden, ids, weights = args
                unweighted = self.candidate_outputs(module, hidden, ids)
                row["routed_delta"] = output.detach()
                row["candidate_unweighted"] = unweighted.detach()
                row["candidate_weighted"] = (
                    unweighted * weights.unsqueeze(-1)
                ).detach()
...
def load_target(model_path: Path) -> tuple[Qwen3_5MoeForCausalLM, Any]:
...
class CaptureRun:
    def __init__(
        ...
        self.events = EventWriter(output / "events.jsonl", run_id)
    ...
    def flush_target_call(
        ...
        *,
        ...
        final_hidden_rows = [
            value.detach() for value in output.hidden_states[-1][0]
        ]
        for local_index, token_id in enumerate(call_input_ids):
            ...
            for layer_id, layer in enumerate(self.model.model.layers):
                ...
                if missing:
                    raise RuntimeError(f"target layer {layer_id} missing hooks {sorted(missing)}")
                index = local_index
                ...
                shared_gate = torch.sigmoid(row["shared_gate_logit"][index]).reshape(1)
                ...
                candidate_sum = weighted_rms.sum()
                fractions = weighted_rms / (candidate_sum + 1e-12)
                candidate_cos = []
                candidate_valid = []
                ...
                layer_event = self.events.emit(
                    ...
                    candidate_unweighted_output_rms=[
                        float(v) for v in unweighted.float().square().mean(dim=-1).sqrt().cpu().tolist()
                    ],
                    ...
                    ready_mtp_node_event_ids=list(self.ready_mtp_nodes),
                ...
                candidate_tensors = {
                    ...
                    "shared_expert_gate_logit": row["shared_gate_logit"][index].reshape(1),
                }
    ...
    def write_mtp_node(
        ...
        *,
        ...
        if audit_key % 100 < self.full_vocab_audit_percent:
        ...
        self.mtp_nodes += 1
        self.ready_mtp_nodes.append(node_event)
...
def capture_sequence(
    ...
    sequence_id = f"tfpilot-seq{sequence_index:04d}"
    request_id = f"tfpilot-req{sequence_index:04d}"
    prompt_token_ids = prompt_ids(tokenizer, prompt["text"])
    sequence_start = run.events.emit(
        ...
        conversation_or_source_family_id=prompt["family"],
        dataset_item_family_id=prompt["family"],
        dataset_source=prompt["source"],
        language_label="en",
        prompt_token_ids=prompt_token_ids,
        prompt_hash=prefix_hash(prompt_token_ids),
        split_group_id=sha256_bytes(prompt["family"].encode()),
    )
    ...
    manifest = {
        ...
        "clock_domain_definitions": {
            "synchronous_transformers_execution_order": (
                "logical host execution order with CUDA synchronization at call boundaries; "
                "not a performance timestamp"
            )
        },
        ...
        "capture_policy": {
            ...
            "all_candidate_vectors_in_pilot": True,
