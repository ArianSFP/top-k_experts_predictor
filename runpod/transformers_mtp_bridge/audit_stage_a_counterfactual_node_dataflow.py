#!/usr/bin/env python3
"""Exercise the real Stage-A node capture -> index -> dataset -> label join path.
...
def main() -> None:
    ...
    expected_shapes = {
        "node_mask": (batch_size, 32),
        "node_local_indices": (batch_size, 32),
        "parent_local_indices": (batch_size, 32),
        "depth": (batch_size, 32),
        "first_divergence_depth": (batch_size, 32),
        "budget_node_masks": (batch_size, 3, 32),
        "budget_endpoint_masks": (batch_size, 3, 32),
        "budget_realized": (batch_size, 3),
        "budget_category_counts": (batch_size, 3, 4),
        "source_edge_logp": (batch_size, 32),
        "source_path_logp": (batch_size, 32),
        "target_edge_logp": (batch_size, 32),
        "target_path_logp": (batch_size, 32),
        "target_next_token_ids": (batch_size, 32),
        "target_next_token_logp": (batch_size, 32),
        "target_next_token_valid": (batch_size, 32),
        "query_coordinates": (batch_size, 32, 40, 255),
        "router_logits": (batch_size, 32, 40, 256),
        "selected_ids": (batch_size, 32, 40, 8),
        "selected_weights": (batch_size, 32, 40, 8),
        "valid": (batch_size, 32, 40),
    }
    observed_shapes = {
    ...
    for split, training in (
        ...
        try:
            load_node_counterfactual_companion(companion, split=split, training=training)
    ...
    report = {
        "schema": "harp_rtt_stage_a_counterfactual_node_dataflow_audit_v3",
        "passed": True,
