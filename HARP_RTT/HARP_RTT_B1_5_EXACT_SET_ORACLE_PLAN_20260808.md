...
1. recompute the immutable four-path B1 artifact using native selected-set inclusion mass;
2. capture every unique adaptive-32 H2--H4 node once;
3. evaluate all-node first, then budgets 16 and 8, with budget 4 exactly backward-compatible;
...
The v3 companion stores one target record per unique parent-before-child tree node:
